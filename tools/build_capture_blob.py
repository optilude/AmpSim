...
def write_if_changed(path, data):
    """Write only when the content differs, so the mtime does not churn.

    src/capture_index.h is an input to every translation unit. Rewriting it
    with identical content on every build bumps its mtime, which makes make
    rebuild the whole project -- turning a no-op `make program-dfu` into a ten
    second compile, long enough to miss the bootloader's DFU window.
    """
    path = Path(path)
...
def build(args):
    ...
    # A same-named .nam and .wav in one directory used to mean "run the NAM
    # into the cabinet IR". That is gone: the pair costs both engines' CPU in
    # the same audio block, which does not fit alongside the reverb. Refuse
    # the build rather than silently picking one, so a .wav that was meant to
    # ship never quietly goes missing.
    conflicts = [
        (model_name, variant_name, files)
        for (model_name, variant_name), files in sorted(model_groups.items())
        if "nam" in files and "ir" in files
    ]
    if conflicts:
        lines = ["Running a NAM and a cabinet IR together is no longer supported."]
        for model_name, variant_name, files in conflicts:
            lines.append(f"  '{model_name}/{variant_name}' has both:")
            lines.append(f"    {files['nam']}")
            lines.append(f"    {files['ir']}")
        lines.append("Rename or remove one file from each pair.")
        raise SystemExit("\n".join(lines))
    ...
    for (model_name, variant_name), files in sorted(model_groups.items()):
        ...
        model_type = "NamOnly" if has_nam else "IrOnly"
    ...
    write_if_changed(args.out_bin, bytes(blob))

    h = io.StringIO()
    h.write("// Auto-generated by tools/build_capture_blob.py\n#pragma once\n\n")
    h.write("#include <cstddef>\n#include <cstdint>\n\n")
    # A model is one engine or the other. Value 2 (NamAndIr) is retired and
    # deliberately not reused, so an old blob cannot be read as a new type.
    h.write("enum class ModelType : uint8_t { NamOnly = 0, IrOnly = 1 };\n\n")
    ...
    write_if_changed(args.out_header, h.getvalue())
    ...
    write_if_changed(args.out_map, m.getvalue())
